...
export const FEATURE_LABELS: Record<FeatureKey, { name: string; blurb: string }> = {
	stats: {
		...
		blurb:
			'Record kills, deaths, cash and results per player per match: leaderboards, careers and the current-match table. One row per player per match, kept a year.'
	},
	publicStatus: {
		name: 'Public status page',
		blurb:
			'A page anyone can open without signing in: map, clock, players, scores, cash and the current match, built from the worker’s last look.'
	},
	publicStats: {
