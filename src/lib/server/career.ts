 ...
 */
function boardCte(ids: string[], from: Date, minMinutes: number): SQL {
	const played = sql`EXTRACT(EPOCH FROM (last_seen - first_seen)) >= ${MIN_PRESENCE_S}`;
	return sql`
		agg AS (
			SELECT steam_id,
			       ...
			       SUM(EXTRACT(EPOCH FROM (last_seen - first_seen))) / 60 AS minutes,
...
export async function loadLeaderboard(
	...
	const [rows, [total]] = await Promise.all([
		env.db.execute<AggRow & { rank: string }>(sql`
			WITH ${boardCte(ids, from, opts.minMinutes)}
...
export async function careerFor(
	...
	const played = sql`EXTRACT(EPOCH FROM (s.last_seen - s.first_seen)) >= ${MIN_PRESENCE_S}`;
	const breakdown = (key: SQL) => sql`
		SELECT ${key} AS key,
		       ...
		       SUM(EXTRACT(EPOCH FROM (s.last_seen - s.first_seen))) / 60 AS minutes
	...
	const [[totals], [ranks], [eligibleRow], maps, factions, recent] = await Promise.all([
		...
		env.db.execute<{
			...
			WITH ${boardCte(ids, from, DEFAULT_MIN_MINUTES)}
			SELECT * FROM (
	...
	const recentRows: CareerMatch[] = recent.map((r) => ({
		...
		deaths: r.deaths,
		cash: r.cash,
		minutes: Math.round((new Date(r.lastSeen).getTime() - new Date(r.firstSeen).getTime()) / 60000),
		counted:
			(new Date(r.lastSeen).getTime() - new Date(r.firstSeen).getTime()) / 1000 >= MIN_PRESENCE_S,
		winner: r.winner
	}));
	if (!totals)
