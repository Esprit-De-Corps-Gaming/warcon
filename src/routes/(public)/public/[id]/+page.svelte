<script lang="ts">
	...
	async function load() {
		try {
			...
			/* keep showing the last good view; the observed-at line says how old it is */
		}
	}
	$effect(() => {
		void id;
	...
	/** the match clock keeps counting from the last look while the server was reachable */
	let clock = $derived.by(() => {
		if (s.matchSeconds === null || !s.observedAt) return null;
		const drift = s.reachable ? Math.max(0, (now - Date.parse(s.observedAt)) / 1000) : 0;
		return s.matchSeconds + drift;
	});
	/** uptime counts up from the game process start */
	let uptime = $derived(
		s.reachable && s.startedAt ? Math.max(0, (now - Date.parse(s.startedAt)) / 1000) : null
	);
	let age = $derived(
		s.observedAt ? Math.max(0, Math.round((now - Date.parse(s.observedAt)) / 1000)) : null
	);
...
{#if !s.reachable}
	<div class="callout mb-4 border-l-danger">
		<b>Unreachable.</b>
		{s.error}
		{#if s.observedAt}Last seen {fmtTime(s.observedAt)}.{/if}
	</div>
{/if}
...
<div class="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4">
	...
	<div class="panel py-4">
		<div class="caps text-mist-400">Uptime</div>
		<div class="mt-1 font-display text-2xl font-semibold tabular">
			{uptime === null ? '—' : fmtDuration(uptime)}
		</div>
		<div class="truncate text-[11.5px] text-mist-600">{expSetLabel(EMPTY, s.experiences)}</div>
	</div>
</div>
...
<div class="panel">
				...
				{:else}
					<tr
						><td colspan="6" class="py-8 text-center text-mist-600"
							>{s.playerCount ? 'Nobody has scored yet.' : 'Nobody is playing right now.'}</td
						></tr
					>
				{/each}
			</tbody>
	...
	<p class="note">
		{#if age !== null}Last seen {age}s ago · this page refreshes every 10s.{:else}Not observed yet.{/if}
	</p>
</div>
