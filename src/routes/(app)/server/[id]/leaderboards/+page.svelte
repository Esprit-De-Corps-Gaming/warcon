...
{#if board}
	...
	<div class="panel">
		...
		<p class="note">
			Built from what RCON reports: kills, deaths and cash per connected player, read as increments
			so a match spanning several sessions, or a reconnect, still counts. A match counts once a
			player was in it for {board.minPresenceMinutes} minutes; win rate needs {board.minDecided}
